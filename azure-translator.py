...
# Modified by Yuval Avidani -> https://github.com/hoodini/azure-translator-python
# ------------------------------------
...
class SampleTranslationWithAzureBlob:
    ...
    def sample_translation_with_azure_blob(self):
        ...
        if self.document_name:
            if self.document_name not in blob_names:
                with open(self.document_name, "rb") as doc:
                    source_container.upload_blob(self.document_name, doc)
            else:
                print(f"Document {self.document_name} already exists in the container.")
        else:
        ...
        target_language = os.getenv("AZURE_TRANSLATE_TARGET_LANGUAGE")
        
        poller = translation_client.begin_translation(source_container_sas_url, target_container_sas_url, target_language)
